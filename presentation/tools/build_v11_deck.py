...
VISUAL_ORDERS = (
    "온디바이스 제약 → K26/Memory FPGA 역할 → 채택 조건 질문",
    "필요성 → 구조 → 검증 → 채택 조건의 네 번호",
    "처음 가설 → 2.43 GiB 용량 계산 → 새 연구 부등식",
    "K26 연산부 → 논리 링크 → Memory FPGA 공급부 → 미구현 물리 경로",
    "데이터 흐름 영상 → 세 타일 사이클 → 3/3 일치 → 시험 범위",
    "C0의 긴 대기열 → C2·C3 유휴 → 지역성과 부하 균형의 충돌",
    "유휴 감지 → 피해 큐 탐색 → 이동 비용 계산 → 소유권 이동 → 완료",
    "7,837개 노드 → 183개 투영 → 802개 TileJob → 의존성 사슬",
    "왼쪽 S1 → 오른쪽 S3 → J79·J90·J100 → 색 범례",
    "합성 부하 p95·p99 → Gemma 기존 배치 → 채널 친화 배치",
...
EXPECTED_QUESTIONS = (
    "이 연구의 핵심 기여를 한 문장으로 말하면 무엇인가?",
    "왜 이 네 질문 순서로 발표하는가?",
    "Gemma 1B가 4 GB에 들어가는데 왜 외부 8 GB를 검토했는가?",
    "현재 실제로 구현된 구간과 후보로 남은 구간은 어디인가?",
    "실제 Gemma가 FPGA에서 실행됐다고 볼 수 있는가?",
    "정적 큐가 지역성을 지키면서도 왜 꼬리 지연을 만드는가?",
    "S3는 어떤 기준으로 작업을 훔치지 않기로 결정하는가?",
    "183개 투영과 802개 TileJob은 어떻게 다른가?",
    "이 타임라인은 실제 Gemma 실행인가?",
    "왜 Gemma 배치에 따라 p95 결과의 방향이 달라지는가?",
...
ANSWER_CORES = (
    "계산·공급 역할 분리, 이동 비용을 포함한 동적 배치, 모델→RTL→KiCad 재현 흐름이 세 기여입니다.",
    "필요성에서 시작해 구조와 검증을 거친 뒤 채택 조건으로 끝내야 결과가 설계 결정으로 연결됩니다.",
    "외부 8 GB는 1B의 필수 용량이 아니라 더 큰 모델·긴 문맥과 가중치 공급 분리를 위한 조건부 확장 후보입니다.",
    "작업 수락부터 논리 응답·링크 FIFO·MatVec 결과까지는 닫혔고 GTH·CDC·MIG 물리 경로는 남아 있습니다.",
    "아닙니다. 실제 가중치 타일 세 개의 논리 RTL 산술만 확인했으며 전체 Gemma 추론은 실행하지 않았습니다.",
    "정적 소유권 때문에 한 큐가 밀려도 빈 클러스터가 돕지 못해 뒤쪽 작업의 대기가 길어집니다.",
    "대기 감소 이득이 가중치·활성값·부분합·링크 이동 비용보다 클 때만 소유권을 옮깁니다.",
    "183은 그래프의 밀집 투영 수이고 802는 이를 N≤1024·전체 K 범위로 나눈 하드웨어 작업 수입니다.",
    "아닙니다. 같은 합성 편향 작업 1,000개와 같은 시드에서 정책만 바꾼 분석 사건입니다.",
    "작업 훔치기는 초기 배치 뒤에 남은 불균형만 줄이므로 기존 산술 배치와 채널 친화 배치의 결과 방향이 달라집니다.",
...
CAUTIONS = (
    "완성 제품·실측 가속기라고 말하지 않는다.",
    "목차를 결과처럼 설명하지 않는다.",
    "외부 8 GB가 Gemma 1B의 필수 용량이라고 말하지 않는다.",
    "논리 폐루프를 물리 GTH·MIG 폐루프로 확대하지 않는다.",
    "세 타일 일치를 전체 모델 실행·정확도로 확대하지 않는다.",
    "정적 큐를 모든 조건에서 나쁜 정책이라고 일반화하지 않는다.",
    "정책 점수를 최적성 증명이나 보드 타이밍으로 말하지 않는다.",
    "그래프 작업 수를 실제 FPGA 실행 횟수라고 말하지 않는다.",
    "분석 사건을 DDR·GTH 물리 사이클로 부르지 않는다.",
    "TileJob p95를 사용자 요청 지연으로 부르지 않는다.",
